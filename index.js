...
// URL Vercel để gửi lệnh sellall (POST { user, cmd })
...
// ===== helper: gọi Vercel để gửi lệnh sellall =====
function callVercelSellall(username, cmd) {
  if (!VERCEL_SELLALL_URL) return;
  const user = String(username || "").trim();
  const command = String(cmd || "").trim().toLowerCase();
  if (!user || !command) return;

  try {
    const urlObj = new URL(VERCEL_SELLALL_URL);
    const body   = JSON.stringify({ user: user, cmd: command });
    ...
    const req = https.request(urlObj, options, (res) => {
      // đọc body để tránh socket hang, nhưng bỏ qua nội dung
      res.on("data", () => {});
    });
    ...
    console.error("Error building Vercel URL:", err);
  }
}
...
// Endpoint cũ cho SAB: { sellall: true/false } (giờ có thể bỏ, nhưng vẫn giữ)
...
function renderKeyPage() {
  // (giữ nguyên code renderKeyPage như file cũ của bạn)
  // mình cắt bớt cho ngắn, bạn chỉ cần copy nguyên phần renderKeyPage
  // từ file hiện tại sang đây là được.
  // ...
}
...
// Step 2: user bấm Confirm -> với sellall thì gọi Vercel, lệnh khác dùng setCommand
app.post("/panel/confirm", (req, res) => {
  ...
  if (cmdRaw === "sellall") {
    // gọi Vercel để gửi .sellall <user> vào Discord (webhook + bot)
    callVercelSellall(userRaw, cmdRaw);
  } else {
    // giữ nguyên behaviour cũ cho các lệnh khác nếu có
