...
// TTL mặc định: 120s (~2 phút), dùng khi user không chọn gì
let COMMAND_TTL_MS = Number(process.env.COMMAND_TTL_MS || 120 * 1000);

// các option cho dropdown duration
...
// In-memory command store: { [userLower]: { cmd, ts, ttlMs } }
const commandStore = Object.create(null);

function setCommand(user, cmd, ttlMsOverride) {
  if (!user || !cmd) return;
  const key = String(user).toLowerCase();
  const effectiveTtl =
    Number(ttlMsOverride) > 0 ? Number(ttlMsOverride) : COMMAND_TTL_MS;
...
function popCommand(user) {
  ...
  const now = Date.now();
  const ttl = Number(entry.ttlMs) > 0 ? Number(entry.ttlMs) : COMMAND_TTL_MS;
...
function getDurationLabel(ttlSeconds) {
  const opt = DURATION_OPTIONS.find((o) => o.value === ttlSeconds);
  if (opt) return opt.label;
  if (!ttlSeconds) return "2 minutes (default)";
  if (ttlSeconds % 86400 === 0) {
    return `${ttlSeconds / 86400} day(s)`;
  }
  if (ttlSeconds % 3600 === 0) {
    return `${ttlSeconds / 3600} hour(s)`;
  }
  if (ttlSeconds % 60 === 0) {
    return `${ttlSeconds / 60} minute(s)`;
  }
...
app.post("/api/set-command", (req, res) => {
  ...
  // optional TTL từ API: ttlSeconds hoặc ttlMs
  const ttlMsFromBody =
    Number(body.ttlMs) ||
    (Number(body.ttlSeconds) > 0 ? Number(body.ttlSeconds) * 1000 : 0);
  ...
  setCommand(user, cmd, ttlMsFromBody);
...
// Đổi TTL mặc định (giây) qua API – dùng cho lệnh admin từ bot
...
function renderPanelPage(userRaw, cmdRaw, state /* "preview" | "sent" */, ttlSecondsRaw) {
  const safeUser = escapeHtml(userRaw);
  const safeCmd = escapeHtml(cmdRaw);

  const isPreview = state === "preview";

  // TTL đang chọn (theo giây)
  let ttlSeconds = parseInt(ttlSecondsRaw, 10);
  if (!Number.isFinite(ttlSeconds) || ttlSeconds <= 0) {
    ttlSeconds = Math.round(COMMAND_TTL_MS / 1000) || 120;
  }

  const ttlLabel = getDurationLabel(ttlSeconds);

  const optionsHtml = DURATION_OPTIONS.map((opt) => {
    const selected = opt.value === ttlSeconds ? " selected" : "";
  ...
  const hintText = isPreview
    ? `After you confirm, the command will be queued for the player. It will stay active for ${ttlLabel}, then expire automatically.`
    : `If the player is in-game and the script is running, it should execute within a few seconds. This command will remain active for ${ttlLabel} from the moment it was created.`;
  ...
  const actionButton = isPreview
    ? `<form method="POST" action="/panel/confirm" style="margin-top:14px;">
         ...
         <div class="duration-hint">
           Choose how long this command should stay active before it expires automatically.
         </div>
...
<body>
  <div class="card">
    ...
    <div class="info">
      ...
      <div><span class="label">Duration:</span> <strong>${ttlLabel}</strong></div>
    </div>
...
app.get("/panel", (req, res) => {
  ...
  const html = renderPanelPage(userRaw, cmdRaw, "preview", null);
...
app.post("/panel/confirm", async (req, res) => {
  ...
  const ttlSeconds = Number(req.body.ttl) || 0;

  if (!userRaw || !cmdRaw) {
    return res.status(400).send("Missing 'user' or 'cmd' in form body.");
  }

  const ttlMs = ttlSeconds > 0 ? ttlSeconds * 1000 : undefined;

  // Lưu lệnh vào store cho SAB (kèm TTL riêng)
  setCommand(userRaw, cmdRaw, ttlMs);
  ...
  const html = renderPanelPage(userRaw, cmdRaw, "sent", ttlSeconds);
  res.setHeader("Content-Type", "text/html; charset=utf-8");
  return res.send(html);
});
